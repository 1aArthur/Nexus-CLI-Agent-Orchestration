    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _MetricChip(
                icon: Icons.token,
                label: 'TOKENS',
                value: agentStatus.activeTokens.toString(),
              ...
              _MetricChip(
                icon: Icons.timer,
                label: 'LATENCY',
                value: '${agentStatus.latencyMs}ms',
              ...
              _MetricChip(
                icon: Icons.speed,
                label: 'PROGRESS',
                value: '${(agentStatus.progress * 100).toInt()}%',
          ...
          if (isPrimary) ...[
            const SizedBox(height: 16),
            Divider(color: NexusTheme.white10),
            const SizedBox(height: 12),
  ...
  List<String> _getCapabilities(AgentType agent) {
    switch (agent) {
      case AgentType.claudeCode():
        return ['Architecture', 'Refactoring', 'Code Review', 'System Design', 'ADR Creation'];
      case AgentType.codex():
        return ['C++20', 'Rust', 'JNI/NDK', 'SIMD', 'CMake', 'Performance'];
      case AgentType.architect():
        return ['Module Graph', 'ADR', 'Patterns', 'Scalability'];
      case AgentType.planner():
        return ['Task DAG', 'Decomposition', 'Scheduling', 'Dependencies'];
      case AgentType.devSecOpsSentinel():
        return ['SAST', 'DAST', 'MASVS', 'OWASP', 'Auto-fix'];
      case AgentType.performanceAgent():
        return ['Profiling', 'Memory', 'SIMD', 'DEX', 'Baseline Profiles'];
      case AgentType.testAgent():
        return ['Unit', 'Integration', 'E2E', 'Property-based', 'Regression'];
      case AgentType.reviewerAgent():
        return ['Diff Analysis', 'Standards', 'Regression Block', 'Security'];
      case AgentType.releaseAgent():
        return ['Changelog', 'Versioning', 'CI/CD', 'Signing', 'Deployment'];
      case AgentType.nativeNdkEngineer():
        return ['C++20', 'Rust', 'JNI', 'CMake', 'SIMD', 'ARM64/x86'];
      case AgentType.exaResearcher():
        return ['Web Search', 'GitHub', 'Tech Intel', 'Grounding'];
      case AgentType.debugAgent():
        return ['Log Parse', 'Stacktrace', 'Hypothesis', 'Root Cause'];
      default:
        return [];
    }
  }
