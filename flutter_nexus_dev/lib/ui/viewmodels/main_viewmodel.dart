...
class MainViewModel extends ChangeNotifier {
  ...
  final ValueNotifier<AgentLiveStatus> _claudeStatus = ValueNotifier(
    AgentLiveStatus(
  ...
  final ValueNotifier<AgentLiveStatus> _codexStatus = ValueNotifier(
    AgentLiveStatus(
  ...
  void _listenToTts() {
    _ttsService.isSpeakingStream.listen((speaking) {
      notifyListeners();
    });
    _ttsService.utteranceStream.listen((utterance) {
      notifyListeners();
    });
  }
  ...
  void _startTelemetryHeartbeat() {
    Future.doWhile(() async {
      ...
      _telemetry.value = TelemetrySnapshot(
        cpuPercent: 12.0 + (random % 20).toDouble(),
        memoryUsageMb: 90.0 + (random % 30).toDouble(),
        activeThreads: 4 + (random % 6),
        latencyMs: 30 + (random % 40),
        totalRequests: _telemetry.value.totalRequests + 1,
        totalTokens: _telemetry.value.totalTokens + 10 + (random % 35),
        sessionCostUsd: _telemetry.value.sessionCostUsd + 0.0001,
      );
      return true;
    });
  }
  ...
  void updateApiKeys({String? gemini, String? exa, String? grok}) {
    ...
    if (grok != null) {
      _customGrokKey.value = grok.trim();
      _grokVoiceService.setApiKey(grok.trim());
    }
  }
  ...
  void executeTerminalCommand(String input) {
    ...
    if (lower == 'zsh' || lower.startsWith('nx shell zsh')) {
      _activeShell.value = TerminalShellType.zsh();
    } else if (lower == 'nu' || lower == 'nushell' || lower.startsWith('nx shell nu')) {
      _activeShell.value = TerminalShellType.nushell();
    } else if (lower == 'fish' || lower.startsWith('nx shell fish')) {
    ...
    _terminalRepository.logCommand(
      command: input,
      output: output,
      isError: false,
      executionTimeMs: duration,
      agentTag: tag,
      shellType: _activeShell.value,
    );
  ...
  void _handleNxCommand(String lower, String originalInput) {
    if (lower.startsWith('nx autopilot') || lower == 'autopilot') {
      final goal = originalInput.replaceFirst('nx autopilot', '').replaceFirst('autopilot', '').trim();
      runAutopilot(goal.isEmpty ? 'Production Delivery' : goal);
    } else if (lower.startsWith('nx debate') || lower.startsWith('debate')) {
      final topic = originalInput.replaceFirst('nx debate', '').replaceFirst('debate', '').trim();
      runDebate(topic.isEmpty ? 'Rust vs C++ NDK SIMD' : topic);
    } else if (lower.startsWith('nx scan') || lower.startsWith('scan') || lower.startsWith('nx security')) {
      runSecurityAudit('CLI Triggered Project', 'val apiKey = "sk-123456789"\nval query = "SELECT * FROM users WHERE id = " + id');
    } else if (lower.startsWith('nx performance')) {
      refreshPerformanceProfile();
    } else if (lower.startsWith('nx deploy') || lower.startsWith('deploy')) {
      triggerPipeline('CLI CI/CD Auto Deployment', 'PRODUCTION');
    } else if (lower.startsWith('nx search') || lower.startsWith('search')) {
      final q = originalInput.replaceFirst('nx search', '').replaceFirst('search', '').trim();
      searchExa(q.isEmpty ? 'Android NDK optimizations' : q);
    } else if (lower.startsWith('nx voice') || lower.startsWith('voice')) {
      final speech = originalInput.replaceFirst('nx voice', '').replaceFirst('voice', '').trim();
      speakText(speech.isEmpty ? 'Nexus Dev Orchestrator system online.' : speech);
    }
  }
  
  void clearTerminal() {
    _terminalRepository.clearHistory();
  }
  
  void setActiveShell(TerminalShellType shell) {
  ...
  void startOrchestration(String goal) {
    ...
    _orchestratorRepository.runOrchestration(
      goal: goal,
      customApiKey: _customGeminiKey.value,
      onAgentStatusUpdate: (agentType, state, task, thoughts) {
        if (agentType == AgentType.claudeCode()) {
          _claudeStatus.value = _claudeStatus.value.copyWith(
            state: state,
            currentTask: task,
            thoughts: thoughts,
          );
        } else if (agentType == AgentType.codex()) {
          _codexStatus.value = _codexStatus.value.copyWith(
            state: state,
            currentTask: task,
            thoughts: thoughts,
          );
  ...
  void runAutopilot(String goal) {
    ...
    _orchestratorRepository.runAutopilotLoop(
      goal: goal,
      customApiKey: _customGeminiKey.value,
      ...
      onAgentStatusUpdate: (agent, state, task, thoughts) {
        if (agent == AgentType.claudeCode()) {
          _claudeStatus.value = _claudeStatus.value.copyWith(state: state, currentTask: task, thoughts: thoughts);
        } else if (agent == AgentType.codex()) {
          _codexStatus.value = _codexStatus.value.copyWith(state: state, currentTask: task, thoughts: thoughts);
        }
        notifyListeners();
      },
    );
  ...
  void runDebate(String topic) {
    _orchestratorRepository.runAgentDebate(topic, _customGeminiKey.value);
  }
  ...
  void runSecurityAudit(String projectName, String codeSnippet) {
    ...
    _devSecOpsRepository.runSecurityAudit(
      projectName: projectName.isEmpty ? 'MobileApp Module' : projectName,
  ...
  void runMasvsStaticAnalysis(String filePath, String codeSnippet) {
    ...
    final result = MasvsStaticAnalyzer.analyzeSourceCode(filePath, codeSnippet);
  ...
  void applyAllMasvsPatches() {
    ...
    final patchedCode = MasvsStaticAnalyzer.applyVerifiedPatches(_currentAuditedCode.value, scan.findings);
    _currentAuditedCode.value = patchedCode;
  ...
  void triggerPipeline(String pipelineName, String targetEnvironment) {
    ...
    _cicdRepository.executePipeline(
      pipelineName: pipelineName.isEmpty ? 'Nexus Continuous Delivery' : pipelineName,
  ...
  void generateNativeModule(String moduleName, String methodName, [String language = 'C++20']) {
    final spec = _cicdRepository.generateNativeModule(
  ...
  void refreshPerformanceProfile() {
    _performanceProfile.value = _cicdRepository.generatePerformanceProfile();
    notifyListeners();
  }
  ...
  // Grok Voice
  Future<void> connectGrokVoice() async {
    await _grokVoiceService.connect();
    notifyListeners();
  }
  
  void disconnectGrokVoice() {
    _grokVoiceService.disconnect();
    notifyListeners();
  }
  
  void sendGrokText(String text) {
    _grokVoiceService.sendTextMessage(text);
  }
  
  void sendGrokAudio(List<int> pcmData) {
    _grokVoiceService.sendAudioChunk(pcmData);
  }
  
  void commitGrokAudio() {
    _grokVoiceService.commitAudio();
  }
